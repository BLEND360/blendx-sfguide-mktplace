...
APP_INSTANCE="${SNOWFLAKE_APP_INSTANCE:-spcs_app_instance_test}"
COMPUTE_POOL="${SNOWFLAKE_COMPUTE_POOL:-pool_nac}"
CONNECTION="${SNOWFLAKE_CONNECTION:-mkt_blendx_demo}"

echo "============================================"
echo "Restarting SPCS Service with Force Pull"
echo "============================================"
echo "Role: $ROLE"
...
SERVICE_EXISTS=$(snow sql -q "USE ROLE $ROLE; SHOW SERVICES IN APPLICATION $APP_INSTANCE;" --connection $CONNECTION 2>&1 | grep -i "blendx_st_spcs" | wc -l | tr -d ' ' || echo "0")

if [ "$SERVICE_EXISTS" -gt "0" ]; then
    echo "Service exists. Using ALTER SERVICE with FORCE_PULL_IMAGE to pull new images..."
